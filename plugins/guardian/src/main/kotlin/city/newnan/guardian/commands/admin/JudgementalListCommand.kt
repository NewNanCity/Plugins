 ...
 * @since 2.0.0
 */
class JudgementalListCommand(private val plugin: GuardianPlugin) {

    @Command("guardian judgemental list|ls")
    ...
    fun judgementListCommand(sender: CommandSender) {
        try {
            ...
            if (players.isEmpty()) {
                ...
                val playerNames = players.joinToString("<gray>, </gray>") { p -> if (p.name != null) "<white>${p.name}</white>" else "<red>?</red>" }
                plugin.messager.printf(sender, playerNames)
            }
