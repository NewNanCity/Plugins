package city.newnan.tpa

import city.newnan.config.extensions.configManager
import city.newnan.core.base.BasePlugin
import city.newnan.core.config.CorePluginConfig
import city.newnan.i18n.extensions.setupLanguageManager
import city.newnan.tpa.commands.CommandRegistry
import city.newnan.tpa.config.TPAConfig
import city.newnan.tpa.i18n.LanguageKeys
import city.newnan.tpa.modules.TPABlockManager
import city.newnan.tpa.modules.TPASessionManager
import java.util.*

/**
 * TPA插件主类
 *
 * 提供现代化的传送请求功能，包括：
 * - TPA/TPAHere传送请求
 * - 玩家屏蔽系统
 * - GUI界面支持
 * - 完整的国际化支持
 * - 配置热重载
 *
 * @author AI Assistant
 * @since 2.0.0
 */
class TPAPlugin : BasePlugin() {

    // ===== 模块声明 =====
    private lateinit var sessionManager: TPASessionManager
    private lateinit var blockManager: TPABlockManager
    private lateinit var commandRegistry: CommandRegistry

    override fun onPluginLoad() {
        logger.info("TPA Plugin loading...")
    }

    override fun onPluginEnable() {
        // 调用重载方法
        reloadPlugin()

        // 初始化模块（按依赖顺序）
        blockManager = TPABlockManager("TPABlockManager", this)
        sessionManager = TPASessionManager("TPASessionManager", this)

        // 初始化命令注册器（不可重载）
        commandRegistry = CommandRegistry(this)
    }

    override fun onPluginDisable() {
        // BasePlugin会自动处理资源清理
    }

    override fun reloadPlugin() {
        try {
            // 1. 清理配置缓存（必需）
            configManager.clearCache()

            // 2. 重新设置语言管理器（必需）
            setupLanguageManager(
                languageFiles = mapOf(
                    Locale.SIMPLIFIED_CHINESE to "lang/zh_CN.yml",
                    Locale.US to "lang/en_US.yml"
                ),
                majorLanguage = Locale.SIMPLIFIED_CHINESE,
                defaultLanguage = Locale.US
            )

            // 3. 重载所有子模块（必需）
            super.reloadPlugin()
        } catch (e: Exception) {
            logger.error(LanguageKeys.Core.Plugin.RELOAD_FAILED, e)
            throw e
        }
    }

    // ===== 配置管理 =====

    /**
     * 获取插件配置
     * 使用标准的touchWithMerge + parse模式
     */
    fun getPluginConfig(): TPAConfig {
        configManager.touchWithMerge("config.yml", createBackup = true)
        return configManager.parse<TPAConfig>("config.yml")
    }

    /**
     * 获取核心配置
     * 使用推荐的getCoreConfig()方法模式
     */
    override fun getCoreConfig(): CorePluginConfig = getPluginConfig().getCoreConfig()

    // ===== 模块访问器 =====

    /**
     * 获取会话管理器
     */
    fun getSessionManager(): TPASessionManager = sessionManager

    /**
     * 获取屏蔽管理器
     */
    fun getBlockManager(): TPABlockManager = blockManager
}
