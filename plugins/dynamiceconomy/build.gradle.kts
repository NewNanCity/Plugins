...
val pluginName = "DynamicEconomy"
version = "2.0.0"
group = "city.newnan.dynamiceconomy"
description = "Dynamic economy system with value resource tracking and commodity pricing"

// 配置插件元数据
tasks.processResources {
    val pluginProperties = createPluginMetadata {
        it.name = pluginName
        it.mainClass = "${project.group}.DynamicEconomyPlugin"
        it.authors = listOf("NewNanCity", "AI")
        it.prefix = pluginName
        it.load = PluginLoadMode.POSTWORLD
        it.dependencies = listOf() // 移除CommandAPI依赖
        it.softDependencies = listOf("Vault", "EssentialsX") // 软依赖Vault和EssentialsX
    }
